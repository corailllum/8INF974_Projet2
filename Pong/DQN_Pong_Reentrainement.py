...
LR            = 1e-4
MEMORY_SIZE   = 50_000
EPSILON_START = 0.05         #on repart de l'epsilon final du premier entraînement
EPSILON_END   = 0.05
EPSILON_DECAY = 0.995
TARGET_UPDATE = 10
FRAME_STACK   = 4
...
def train():
    ...
    #chargement du modèle préentraîné sur 500 épisodes
    print(" Chargement du modèle v1...")
    agent.policy_net.load_state_dict(torch.load("dqn_pong.pth"))
    agent.target_net.load_state_dict(torch.load("dqn_pong.pth"))
    print(" Modèle v1 chargé, reprise de l'entraînement...")

    print(f"  Entraînement DQN sur Pong Atari (suite)")
    print(f"  Actions disponibles : {n_actions}")
    print(f"  Épisodes supplémentaires : {EPISODES}")
    ...
    torch.save(agent.policy_net.state_dict(), "dqn_pong_v2.pth")
    print(" Modèle sauvegardé : dqn_pong_v2.pth")

    return agent

#démo finale avec affichage pygame et enregistrement vidéo v2
def demo(agent, video_path="pong_demo_v2.mp4"):
    print("\nLancement de la démo finale v2")
    ...
    pygame.display.set_caption("DQN Pong v2 - Démonstration")
